...
class PrePrintedForm(models.Model):
    ...
    name = fields.Char(
        ...
        required=True,
        default=lambda self: self.env["ir.sequence"].next_by_code("pre.printed.form")
        or "New",
    )
    ...
    text_item_ids = fields.One2many(
        comodel_name="overlay.text.item", inverse_name="form_id", string="Text Items"
    )
    ...
    def process_action(self, record_id):
        record_container = self.env[self.model_id.model].search(
        ...
        #     "pre-printed-forms-writer", "static/fonts", "AGENCYB.TTF"
        # )

        # Register custom fonts
        # pdfmetrics.registerFont(TTFont("AgencyFB", font_path))
        # pdfmetrics.registerFont(
        #     TTFont("AgencyFB-Bold", font_path)
        # )  # Register the same font for bold if no separate bold TTF is available
        ...
        buffer = BytesIO()
        overlay_pdf = canvas.Canvas(
            buffer, pagesize=PAGE_SIZES.get(self.page_size, letter)
        )
